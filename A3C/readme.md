...
$$
   ...
   \[
   \text{Here, } \alpha \text{ represents the learning rate, } \pi_{\theta}(a|s) \text{ denotes the probability distribution of taking action } a \text{ given state } s, \text{ and } A(s,a) \text{ is the advantage function that measures how much better it is to take action } a \text{ compared to average behavior.}
   \]
...
\[
   \text{Where } \beta \text{ is the learning rate, } R \text{ is the reward, and } V_{\theta}(s) \text{ is the value function indicating the expected return starting from state } s \text{ following policy.}
   \]
