...
class DuelingDQNDNN_TLSAgent:
    ...
    def neural_network(self, name):
        ...
        return model, target_model  # 返回构建好的神经网络模型和目标网络模型

    def model_predict(self, state):
        """模型预测"""
        state = np.expand_dims(state, axis=0)  # 将状态扩展为神经网络接受的格式
        output = self.model.predict(state)  # 使用模型预测动作值
        return output  # 返回模型预测的输出
    ...
    def experience_replay(self):
        ...
        for state, action_index, reward, next_state, done in mini_batch:
            ...
            if done:
                ...
                target = reward + gamma * np.max(
                    self.target_model.predict(next_state)[0])  # 否则，目标值为即时奖励加上折扣因子乘以下一状态的最大动作值的估计值

            target_t = self.model.predict(state)  # 使用当前模型预测当前状态的动作值
            target_t[0][action_index] = target  # 更新目标值

            # 打印调试信息
            print(f"State: {state}, Action: {action_index}, Reward: {reward}, Next State: {next_state}, Done: {done}")
            print(f"Target: {target}, Target_t: {target_t}")
        ...
        sd.save_loss(self.name, loss)  # 保存本次训练损失
