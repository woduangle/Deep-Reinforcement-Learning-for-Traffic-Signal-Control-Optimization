...
from keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, Activation, Dropout, SimpleRNN, LSTM, GRU, Input, Lambda
...
class Follower_Agent:
    ...
    def __init__(self, *args):
        ...
        self.master = agent_setting['asymmetric_opponent']
        """深度学习超参数"""
    ...
    def build_dnn(self):
        ...
        fc1 = Dense(units=units, activation='relu')(inputs)
        fc2 = Dense(units=units, activation='relu')(fc1)
        # Split into value and advantage streams
        val_stream = Dense(units=1, activation='linear')(fc2)
        adv_stream = Dense(units=output_dim, activation='linear')(fc2)
        # Combine streams into Q-value output
        q_vals = Lambda(lambda x: x[0] + x[1] - tf.reduce_mean(x[1], axis=1, keepdims=True))([val_stream, adv_stream])

        model = Model(inputs=inputs, outputs=q_vals)
    ...
    def memorize(self, done=False):
        ...
        self.reply_memory.append(
            (self.state_prev, self.action_current_index, self.reward_current, self.state_current, self.exp_action_list,
             done))
    ...
    def experience_replay(self):
        ...
        for state, action_index, reward, next_state, action_list, done in mini_batch:
            ...
            se_value = ng.get_agents_SE_value_by(name=self.name, follower_action_list=action_list)
            print(se_value)
            """calculate asy value of agents"""
            # SE_values_dic = get_leader_agent_SE_value_by(name, Asy_payoff_table_dic, action_dic, agents_setting)
            """give game value"""
            se_q_value = se_value
            ...
            if done:
                ...
                target = reward + gamma * se_q_value  # 计算目标值

            target_t = self.model.predict(state)
            target_t[0][action_index] = target
            # filtering out states and targets for training
            state_f.append(state[0])
            target_f.append(target_t[0])
            #
            ng.update_agent_SE_payoff_table(self.name, payoff=target, action_list=action_list)
