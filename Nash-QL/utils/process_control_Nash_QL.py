...
def update_payoff_table(tls_agents, agent):
    """更新收益矩阵"""
    try:
        ng.update_leader_NE_payoff_table(leader_name=agent,
                                         opponent_name=tls_agents[agent].opponent,
                                         my_action=tls_agents[agent].action_current,
                                         your_action=tls_agents[tls_agents[agent].opponent].action_current,
                                         my_q=tls_agents[agent].get_q_value(),
                                         your_q=tls_agents[tls_agents[agent].opponent].get_q_value())
    except KeyError:
        pass
