from init.init_NASH_Stackelberg import *
    ...
    for ep in range(basic_setting['episode']):
        ...
        while get_env_time() < start_time + basic_setting['simulation_time']:
            ...
            for agent in agent_setting.keys():
                ...
                # 获取状态
                get_current_state(agent=tls_agents[agent])
            # 计算nash中leader的nashQ值
            ng.get_leaders_NE_values_by()
            # 计算Stackelberg中的q值
            ng.get_agents_SE_value_by(agent_setting=agent_setting, tls_agents=tls_agents)
            # 遍历所有Agent
            for agent in agent_setting.keys():
                # 重新获取Agent的动作执行情况，是否完成
                if tls_agents[agent].is_current_strategy_over() == False:  # 当前动作执行中。。。
            ...
            # 更新nash收益矩阵
            update_ne_payoff_table(tls_agents=tls_agents)
            # 更新Stackelberg收益矩阵
            ng.update_agents_SE_payoff_table(agent_setting=agent_setting, tls_agents=tls_agents)
